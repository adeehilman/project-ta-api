...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (Auth::attempt($credentials)) {
             ...
             */
            $isUUIDMatching = DB::select("SELECT * FROM tbl_mms WHERE badge_id = '$request->badge_id' AND UUID = '$request->uuid_new' AND versi_aplikasi = '$request->versi_aplikasi' LIMIT 1");
            ...
            if (count($isUUIDMatching) > 0) {
                ...
                if ($isUUIDMatching[0]->is_new_uuid == '0') {
                    ...
                    try {
                        ...
                        // update player id di tabel karyawan
                        DB::table('tbl_karyawan')
                            ->where('badge_id', $request->badge_id)
                            ->update([
            ...
            if ($list_device_karyawan[0]->jlh > 1) {
                /**
                ...
                try {
                    ...
                    return response()->json([
                        "message" => "Kamu belum bisa login, kami akan bantu kamu agar bisa login, silahkan login kembali selama 2 X 24 Jam",
                    ], 400);
                } catch (\Throwable $th) {
            ...
            try {
                ...
                /**
                 * cek terlebih dahulu apakah user sudah pernah
                 * migrasi dari aplikasi satnusa lama ke satnusa baru
                 */
                $isUserReplaced = DB::select("SELECT is_new_uuid FROM tbl_mms WHERE badge_id = '$request->badge_id' AND is_new_uuid = '1' AND versi_aplikasi = '$request->versi_aplikasi'");
                if ($isUserReplaced) {
                ...
                DB::rollBack();
                return response()->json([
                    "message" => "UUID Sudah digunakan, silahkann hubungi HRD",
                ], 400);
            }
        } else {
            return response()->json([
    ...
    private function checkSecurityPhone($decypt_text, $status_toggle)
    {
        ...
        if ($decypt_text != false || $decypt_text != null) {
            ...
            $query = "SELECT a.badge_id,  b.fullname, uuid, status_pendaftaran_mms, dept_code, position_code, img_dpn, img_blk, img_user FROM tbl_mms  AS a JOIN tbl_karyawan AS b ON b.badge_id = a.badge_id WHERE UUID = '$uuid' OR imei1 = '$uuid' OR imei2 = '$uuid' LIMIT 1";
    ...
    private function checkBarcodeLabelNew($barcode, $status_toggle)
    {
        ...
        if ($jlh ==  0) {
            ...
            $query_lms = "SELECT a.badge_id, fullname, dept_code, position_code, img_dpn, img_blk, img_user, stat_title AS STATUS FROM tbl_lms AS a
            JOIN tbl_karyawan AS b ON a.badge_id = b.badge_id
